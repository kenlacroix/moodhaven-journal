...
**v0.9.4** *(in progress)* — Landing site visual overhaul: hero screenshot depth treatment with glow and layered shadow, download page trust strip with version badge and SHA-256 callout, homepage section spacing pass for clearer visual hierarchy, NavBar forced-white background on load (was invisible on light hero), typography scale pass across all headings, additional app screenshots in context (writing view, calendar, settings).
