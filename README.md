...
| Writing appearance drawer — font, size, line height, tint | Activity tagging — tag entries with 15 predefined activities + custom; mood correlation chart in Insights |
...
**v1.8.0** — Activity tagging: tag journal entries with 15 predefined activities (Exercise, Social, Work, Reading, etc.) and up to 50 custom activities. Activity correlation chart in Insights shows diverging bars for per-activity mood delta vs overall average. Timeline filter bar lets you filter entries by activity. Browser/PWA full parity via IndexedDB v3 with activities object store. 1,467 tests.
