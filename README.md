...
| Platform | Installer |
|:---|:---|
| Windows | `MoodHaven_1.3.0_x64-setup.exe` |
| macOS | `MoodHaven_1.3.0_x64.dmg` |
| Linux | `moodhaven_1.3.0_amd64.AppImage` or `.deb` |
...
|:---|:---|:---|
| **Windows** | `MoodHaven_1.3.0_x64-setup.exe` | Windows 10 |
| **macOS** | `MoodHaven_1.3.0_x64.dmg` | macOS 10.15 Catalina |
| **Linux** | `moodhaven_1.3.0_amd64.AppImage` | Any modern distro |
| **Linux (Debian)** | `moodhaven_1.3.0_amd64.deb` | Ubuntu 22.04+ |
...
| **Testing** | [Vitest](https://vitest.dev) + Testing Library · 1165 tests |
...
**v1.2.1** — Security hardening: TOTP secrets encrypted at rest (AES-256-GCM, amber migration banner for v1.1.x users), path traversal fix in media storage, writer window scoped to ~30 commands only, backend rate limiting on `verify_password`, settings sync allowlist, full-restore SHA-256 integrity check, CSP narrowed.
