...
/// Capped at one restore chunk (4 MiB) + AES-GCM overhead — a malicious restore
/// server cannot force the client to allocate more than a single chunk per frame.
pub fn read_frame_enc_binary(stream: &mut TcpStream, key: &[u8; 32]) -> Result<Vec<u8>, String> {
    // 4 MiB chunk plaintext + 28 bytes AES-GCM overhead (12 nonce + 16 tag)
    let encrypted = read_frame_bytes_with_limit(stream, 4 * 1024 * 1024 + 28)?;
