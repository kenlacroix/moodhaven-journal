/**
 * EntryActionsMenu - Three-dots (⋯) actions dropdown for timeline entries
 *
...
export function EntryActionsMenu({ entry, onDelete, onPinToggle, onSealEntry }: EntryActionsMenuProps) {
  ...
  const handleCopyText = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation();
    const titleLine = entry.title ? `${entry.title}\n\n` : '';
    const body = htmlToPlainText(entry.content);
    await navigator.clipboard.writeText(titleLine + body);
  ...
  }, [confirmingDelete, entry.id, onDelete]);
      ...
      <button
        ...
        className={`flex items-center justify-center w-7 h-7 rounded-md text-sm font-bold tracking-widest transition-all duration-150 ${
          open
            ...
            : 'opacity-0 group-hover:opacity-100 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
        }`}
