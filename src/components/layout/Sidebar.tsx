 ...
 * Per UX spec:
 * - Always visible
 * - Fixed order: Timeline, Search, On This Day, Insights
 * - Settings gear at bottom (separated)
 * - No badges, counters, charts, notifications, inline previews
...
export function Sidebar({ currentView, onNavigate, onLock }: SidebarProps) {
  ...
  return (
    <aside className={`relative flex-shrink-0 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-all duration-300 ${collapsed ? 'w-[68px]' : 'w-64'}`}>
      {/* Collapse toggle */}
      <button
        type="button"
      ...
      {/* Main navigation - fixed order per spec */}
