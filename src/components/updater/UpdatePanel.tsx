...
export function UpdatePanel({ hook, currentVersion }: UpdatePanelProps) {
  ...
  // Security updates (running version below the 1.8.0 encryption floor) are
  // prominent and non-skippable.
  ...
  return (
    <div className="space-y-4">
      ...
      {!isChecking && updateInfo?.is_available && phase === 'idle' && (
        <div className={`rounded-2xl border bg-white dark:bg-slate-900 shadow-sm overflow-hidden ${isSecurity ? 'border-rose-300 dark:border-rose-700' : 'border-violet-200 dark:border-violet-800'}`}>
          {/* Security banner — non-skippable, prominent */}
          {isSecurity && (
            <div className="flex items-start gap-2 px-5 py-3 bg-rose-50 dark:bg-rose-900/30 border-b border-rose-200 dark:border-rose-700 text-rose-700 dark:text-rose-300">
              <svg className="w-5 h-5 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
              </svg>
              <div>
                <p className="text-sm font-bold">Security update — please install</p>
                <p className="text-xs mt-0.5 opacity-90">
                  This update encrypts your journal at rest. Your current version stores data
                  unencrypted — updating is strongly recommended.
                </p>
              </div>
            </div>
          )}
          {/* Header */}
          <div className={`flex items-center justify-between px-5 py-4 border-b ${isSecurity ? 'bg-rose-50/60 dark:bg-rose-900/10 border-rose-100 dark:border-rose-800' : 'bg-violet-50 dark:bg-violet-900/20 border-violet-100 dark:border-violet-800'}`}>
            <div>
              <p className={`text-sm font-semibold ${isSecurity ? 'text-rose-700 dark:text-rose-300' : 'text-violet-700 dark:text-violet-300'}`}>
                {updateInfo.version} available
              </p>
              {updateInfo.pub_date && (
                <p className={`text-xs mt-0.5 ${isSecurity ? 'text-rose-500 dark:text-rose-400' : 'text-violet-500 dark:text-violet-400'}`}>
          ...
          <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-3">
            {updateInfo.can_self_update && updateInfo.asset && !isBrowser ? (
              <>
                ...
                {/* Security updates are non-skippable */}
                {!isSecurity && (
                  <button
                    type="button"
